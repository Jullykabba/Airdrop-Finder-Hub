...
import { Search, Filter, Zap, Coins } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="pb-16">
      {/* Hero Section */}
      <section className="relative overflow-hidden bg-[#060c1a] pt-20 pb-24 border-b border-blue-900/30">
        <div className="absolute inset-0 z-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[350px] bg-blue-600/20 blur-[130px] rounded-full" />
          <div className="absolute top-0 right-0 w-[350px] h-[350px] bg-indigo-500/10 blur-[100px] rounded-full" />
          <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-blue-400/5 blur-[80px] rounded-full" />
        </div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <Badge className="mb-6 bg-blue-600/10 text-blue-400 border-blue-500/20 hover:bg-blue-600/20 px-4 py-1.5 text-sm font-medium rounded-full">
            <Zap className="w-4 h-4 mr-2 inline-block" />
            Track. Discover. Earn.
          </Badge>

          <h1 className="text-4xl md:text-6xl font-display font-bold text-white mb-6 text-balance mx-auto leading-tight">
            Find the Best{" "}
          ...
          <p className="text-sm text-blue-400/70 tracking-widest uppercase font-medium">
            {SITE_NAME} — Powered by Web3
          </p>
        </div>
      </section>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide shrink-0 items-center pl-2 md:border-l border-border md:pl-4">
            ...
            {networks.map((net) => (
              <Button
                ...
                data-testid={`button-filter-${net.toLowerCase()}`}
              >
                {net}
              </Button>
            ))}
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-[280px] rounded-2xl bg-slate-100 dark:bg-slate-800 animate-pulse"
              />
            ))}
          </div>
        ) : !airdrops?.length ? (
